...
PersimmonDemoWin::PersimmonDemoWin()
	: Window("button")
{
	...
	timer->timeout.connect(this, &PersimmonDemoWin::onButton);
	timer->start();
}
...
void PersimmonDemoWin::onButton(void)
{
	...
	for (;;)
	{
		...
		int16_t position_point_x = rand() % (480 - 190);

		srand((unsigned int)rt_tick_get());
		int16_t position_point_y = rand() % (320 - 74);
		...
		if (is_press == true)
		{
			point_des_x = touch_point_x - 95;
			point_des_y = touch_point_y - 37;
		}
...
bool PersimmonDemoWin::handleGestureEvent(struct rtgui_event_gesture *gev,
	const struct rtgui_gesture *gest)
{
	...
	rt_kprintf("touch_point_x %d, touch_point_y %d \n", touch_point_x, touch_point_y);
