 ...
 * Change Logs:
 * Date           Author       Notes
 * 2018-08-13     RT-Thread    the first verion
 */
...
static const struct wifi_cmd_des debug_tab[] = 
{
    ...
    {"autoc", wifi_debug_set_autoconnect},
};
...
static int wifi_debug_help(int argc, char *argv[])
{
    ...
    rt_kprintf("autoc enable/disbale\n");
    return 0;
}
...
static int wifi_debug_set_prot(int argc, char *argv[])
{
    ...
    if (rt_strcmp(argv[1], "lwip") == 0)
    {
        prot = RT_WLAN_PROT_LWIP;
        rt_wlan_prot_attach(argv[2], prot);
    }
    return 0;
}
